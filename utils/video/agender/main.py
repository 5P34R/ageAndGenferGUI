...
if (face_detector_kind == 'haar'):
    face_cascade = cv.CascadeClassifier('utils/video/agender/face_haar/haarcascade_frontalface_alt.xml')
else:
    face_net = cv.dnn.readNetFromTensorflow('utils/video/agender/face_net/opencv_face_detector_uint8.pb', 'utils/video/agender/face_net/opencv_face_detector.pbtxt')
...
if (age_gender_kind == 'ssrnet'):
    ...
    gender_net.load_weights('utils/video/agender/age_gender_ssrnet/ssrnet_gender_3_3_3_64_1.0_1.0.h5')

    # Initialize age net
    age_net = SSR_net(face_size, stage_num, lambda_local, lambda_d)()
    age_net.load_weights('utils/video/agender/age_gender_ssrnet/ssrnet_age_3_3_3_64_1.0_1.0.h5')
else:
    # Setup global parameters
    face_size = 227
    face_padding_ratio = 0.0
    # Initialize gender detector
    gender_net = cv.dnn.readNetFromCaffe('utils/video/agender/age_gender_net/deploy_gender.prototxt', 'utils/video/agender/age_gender_net/gender_net.caffemodel')
    # Initialize age detector
    age_net = cv.dnn.readNetFromCaffe('utils/video/agender/age_gender_net/deploy_age.prototxt', 'utils/video/agender/age_gender_net/age_net.caffemodel')
